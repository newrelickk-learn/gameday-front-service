...
public class CatalogueController {
    ...
    public Product[] home(Principal principal, @RequestParam String tags) throws CatalogueClientException {
    ...
    public Product getImage(Principal principal, @PathVariable("id") String id) throws CatalogueClientException {
        User user = userService.getUserByPrincipal(principal);
    ...
    public Product getProduct(Principal principal, @PathVariable("id") String id) throws CatalogueClientException {
        User user = userService.getUserByPrincipal(principal);
        return client.get(id, user);
    }

    @GetMapping(value={"/catalogue/tags"}, produces = "application/json")
    public Tags getTags(Principal principal) throws CatalogueClientException {
        userService.getUserByPrincipal(principal);
        return client.getTags();
    }

    @PostMapping(value={"/catalogue/search"}, produces = "application/json")
    public SearchResponse search(Principal principal, @RequestBody SearchRequest searchRequest) throws CatalogueClientException, ExecutionException, InterruptedException, JsonProcessingException {
        User user = userService.getUserByPrincipal(principal);
